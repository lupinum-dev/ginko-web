...
function validateASTStructure(node: any): boolean {
  if (!node || typeof node !== 'object') return false
  if (!node.type) return false

  if (Array.isArray(node.content)) {
    return node.content.every(validateASTStructure)
  }
  return true
}
...
::`
  ...
  // Integration tests for the entire parser
  describe('Parser Integration', () => {
    it('correctly parses content with mixed elements', () => {
      const input = `
# Heading

::note(important=true)
This is a note with \`inline code\` and a dash element:
--item(checked=true) Task item
::
...
`;
      ...
      // Validate structure
      expect(validateASTStructure(result)).toBe(true);
      ...
      // Find the note block
      const noteBlock = content.find(node =>
        node.type === 'block' && (node as BlockNode).name === 'note'
      ) as BlockNode;

      expect(noteBlock).toBeDefined();
      expect(noteBlock.properties).toHaveLength(1);
      expect(noteBlock.properties[0]).toEqual({ name: 'important', value: true });

      // Verify the dash element inside the note
      const dashElement = noteBlock.content.find(node =>
        node.type === 'dash-element'
      ) as DashElementNode;

      expect(dashElement).toBeDefined();
      expect(dashElement.name).toBe('item');
      expect(dashElement.properties).toHaveLength(1);
      expect(dashElement.properties[0]).toEqual({ name: 'checked', value: true });

      // Verify code block is present
      const codeBlock = content.find(node =>
...
::`;
      ...
      // Validate outer block
      expect(result.content).toHaveLength(1);
      const outer = result.content[0] as BlockNode;
      expect(outer.type).toBe('block');
      expect(outer.name).toBe('outer');

      // Validate middle block
      expect(outer.content).toHaveLength(1);
      const middle = outer.content[0] as BlockNode;
      expect(middle.type).toBe('block');
      expect(middle.name).toBe('middle');

      // Validate inner block
      expect(middle.content).toHaveLength(1);
      const inner = middle.content[0] as BlockNode;
      expect(inner.type).toBe('block');
      expect(inner.name).toBe('inner');

      // Validate dash elements
      expect(inner.content).toHaveLength(2);
      const item1 = inner.content[0] as DashElementNode;
      const item2 = inner.content[1] as DashElementNode;

      expect(item1.type).toBe('dash-element');
      expect(item1.name).toBe('item1');
      expect(item1.label).toBe('First');

      expect(item2.type).toBe('dash-element');
      expect(item2.name).toBe('item2');
      expect(item2.label).toBe('Second');
    });

    it('correctly preserves empty lines in text nodes', () => {
...
empty lines`;
      ...
      const result = parseMarkdown(input) as DocumentNode;
      const textNode = result.content[0] as TextNode;

      expect(textNode.type).toBe('text');
      expect(textNode.content).toBe(input);
    });

    it('handles blocks with only whitespace content', () => {
      const input = `::block
...
::`;
      ...
      // The block may or may not have content depending on implementation
      // Some parsers might strip empty whitespace nodes
      if (block.content.length > 0) {
        const textNode = block.content[0] as TextNode;
        expect(textNode.type).toBe('text');
        expect(textNode.content).toBe('\n');
      }
    });

    it('processes content with indentation properly', () => {
      const input = `::block
  Indented text
  --item Indented item
    More indentation
::`;
      ...
      // First node should be text with indentation preserved
      const textNode = block.content[0] as TextNode;
      expect(textNode.type).toBe('text');
      expect(textNode.content).toBe('  Indented text\n  ');

      // Second node should be the dash element
      const dashNode = block.content[1] as DashElementNode;
      expect(dashNode.type).toBe('dash-element');
      expect(dashNode.name).toBe('item');
      expect(dashNode.label).toBe('Indented item');
      expect(dashNode.content).toHaveLength(1);

      // Content of dash element should preserve indentation
